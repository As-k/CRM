   ...
   public static Item opp1;
    ...
    private DragItemStartCallback mDragItemStartCallback = new DragItemStartCallback() {
        ...
        public void endDrag(View itemView, int originalPosition,int originalColumn, int newPosition, int newColumn) {
            TextView pk = itemView.findViewById(R.id.pk);


                String PK = pk.getText().toString();
                serverUrl = new ServerUrl(getContext());
                client = serverUrl.getHTTPClient();
                RequestParams params = new RequestParams();
                if (newColumn== 0){
                    params.put("state","contacted");
                }else if (newColumn== 1){
                    params.put("state","demo");
                }
                else if(newColumn==2){
                    params.put("state","requirements");
                }
                else  if(newColumn==3){
                    params.put("state","proposal");
                }
                else if(newColumn==4){
                    params.put("state","negotiation");
                }
                else if(newColumn==5){
                    params.put("state","conclusion");
                }


                client.patch(ServerUrl.url + "/api/clientRelationships/deal/"+ PK +"/", params, new JsonHttpResponseHandler() {
                    @Override
...
//                    if(actionSuccessful && MainActivityOpp.data.add(new SimpleBoardAdapter.SimpleColumn("Contacting", (ArrayList) list))){
//                        RequestParams params = new RequestParams();
//                        params.put("state","contacted");
//                        client.patch(ServerUrl.url + "/api/clientRelationships/deal/" + dealPk +"/", params, new AsyncHttpResponseHandler(){
//                            @Override
//                            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
//                                Toast.makeText(context,"Patch Successful at 'Contacting' ",Toast.LENGTH_SHORT).show();
//                            }
//
//                            @Override
//                            public void onFailure(int statusCode, Header[] headers, byte[] responseBody, Throwable error) {
//                                Toast.makeText(context,"Patch Failed",Toast.LENGTH_SHORT).show();
//                            }
//                        });
//                    }
            }
